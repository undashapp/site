...
- **Eine Tabelle öffnen.** Ein Klick auf eine Tabelle öffnet ein neues Modell mit `SELECT * FROM <table>`. Es wird gespeichert, wenn Sie es speichern oder wenn Sie sein erstes Diagramm speichern.
...
Die Abfrage eines Modells ist eine DuckDB-`SELECT`-Abfrage. Sie darf Tabellen und andere Modelle über deren Namen referenzieren und CTEs (`WITH ...`) verwenden. *undash* führt die Abfrage beim Speichern aus und lehnt sie ab, wenn

- sie überhaupt keine Tabelle und kein Modell referenziert,
- sie eine Tabelle oder ein Modell referenziert, das nicht existiert, oder das Modell selbst,
...
Das SQL eines gespeicherten Modells lässt sich nachträglich nicht mehr ändern. Um es zu ändern, legen Sie ein neues Modell mit der neuen Abfrage an.
...
Speichern Sie mit der Schaltfläche zum Speichern oder mit `s`. Beim Speichern eines Diagramms wird auch das Modell gespeichert.
...
Modelle erscheinen in der Liste **Modelle** der Datenübersicht. Hat ein Modell den Fokus, zeigt seine Karte außerdem, wie viele Transformationen, Filter, Steuerelemente, Dimensionen, Metriken und Diagramme es enthält.

| Aktion | So geht's |
|---|---|
| **Öffnen** | Klicken Sie auf das Modell, oder drücken Sie `Enter`. |
| **Bearbeiten** | ⋮-Menü → **Bearbeiten**, oder `e`. Ändert Titel und Namen. Diagramme und Dashboard gehen mit dem Namen mit. Der Name ist gesperrt, solange ein anderes Modell von diesem abhängt. |
| **Exportieren** | ⋮-Menü → **Exportieren**, oder `d`. Lädt das Ergebnis der Abfrage des Modells herunter. |
...
Der Bereich **Abfrage** ist standardmäßig eingeklappt. Er zeigt das SQL des Modells. Klicken Sie darauf, um die Abfrage zu kopieren. Die Symbole in seiner Kopfzeile bieten drei Werkzeuge.

### Stichproben

Bei Modellen mit 200.000 oder mehr Zeilen erscheint ein Reagenzglas-Symbol. Sind Stichproben aktiviert (Symbol rot, oder drücken Sie `x`), werden alle Diagramme aus den ersten 100.000 Zeilen des Modells statt aus allen Zeilen berechnet, und das Symbol zeigt, welchem Anteil der Daten das entspricht. Stichproben sind nützlich, um große Datenmengen schnell zu erkunden.
...
Die DB-Konsole ist ein SQL-Editor mit Ergebnistabelle. Öffnen Sie sie mit der Konsolen-Schaltfläche in der unteren Leiste, mit `Ctrl+C` oder durch Eingabe von `v` und dann `t`. Drücken Sie `Ctrl+T`, um den Tastaturfokus zwischen Seitenleiste und Konsole zu wechseln.

Die Konsole führt Ihre Abfrage **während der Eingabe** aus. Sie zeigt, wie lange die Abfrage gedauert hat, und eine Ergebnistabelle mit bis zu 1.000 Zeilen (bei breiten Ergebnissen weniger). Schlägt die Abfrage fehl, erscheint ein rotes Symbol. Klicken Sie darauf, oder drücken Sie `Ctrl+I`, um den Fehler zu sehen.
...
- In den Diagrammbereichen zeigt sie die Abfrage hinter dem aktuellen Diagramm und deren Ergebnis. So werfen Sie einen Blick auf das SQL hinter einem Diagramm.

Sobald Sie die Abfrage bearbeiten, wird sie zu einer gewöhnlichen, eigenen Abfrage.
